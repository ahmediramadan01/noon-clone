        ...
        >
            {links.map((link, index) => (
                <SwiperSlide key={index}>
                    <a href={link.href} className={`nav-link text-sm md:text-base lg:text-md text-nowrap block text-center${index === links.length - 1 ? ' text-red-500 font-semibold' : ''}`}>{link.title}</a> {/* Responsive font size and text centering */}
                </SwiperSlide>
            ))}
        </Swiper>
    );
