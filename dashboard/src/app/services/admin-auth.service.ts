...
export class AdminAuthService {
  ...
  login(email: any, password: any): Observable<object> {
    ...
    return this.httpClient
      .post<IUser>(
        `${environment.BAseApiURL}/auth/signin`,
        { x },
        // user,
        this.httpHeader
      )
  ...
  sendCode(email) {
    return this.httpClient
      .post(`${environment.BAseApiURL}/emailRecovery`, { email })
      .pipe(retry(3));
  }
  verifyCode(enteredCode) {
    return this.httpClient
      .post(`${environment.BAseApiURL}/resetCode`, { enteredCode })
      .pipe(retry(3));
  }
  resetPassword(userPassword, enteredCode) {
    return this.httpClient
      .patch(`${environment.BAseApiURL}/resetPassword`, {
        userPassword,
        enteredCode,
      })
      .pipe(retry(3));
