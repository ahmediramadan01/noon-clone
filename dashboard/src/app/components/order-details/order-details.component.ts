...
export class OrderDetailsComponent {
  ...
  toggleStatus(status: string) {
    this.orderService
      .toggleStatus(this.order._id, status, this.order.paymentStatus)
      .subscribe({
        next: (data) => {
          console.log(data);
  ...
  cancelOrder() {
    this.orderService
      .cancelOrder(this.order?._id, this.order?.paymentStatus)
      .subscribe({
        next: (data) => {
          console.log(data);
          this.ngOnInit();
  ...
  orderCancelPrds() {
    for (let i = 0; i < this.itemsIds.length; i++) {
      this.orderService.orderCanceledPrds(
        this.itemsIds[i].id,
        this.itemsIds[i].quantity
      );
    }
  }
}
