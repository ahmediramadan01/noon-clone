...
export class EditproductComponent implements OnInit {
	...
	ngOnInit() {
		this.activetedRout.paramMap.subscribe((paramMap) => {
			...
			this.productsService.GetProductById(this.currentproduct).subscribe((data) => {
				console.log(data);
	...
	updateProduct() {
		...
		if (this.file[0]) {
			...
			this.productsService.uploadImage(thumbnailFormData).subscribe((thumbnailRes) => {
				if (thumbnailRes) {
	...
	updateProductAndNavigate() {
		this.productsService.updateProduct(this.currentproduct, this.product).subscribe({
			next: (data) => {
